...
import { getFiatMetricSelector, queryFiatMetrics } from '../../services/slices/dashboard/fiatMetrics'
import { getFiatRevenueSelector, queryFiatRevenue } from '../../services/slices/dashboard/fiatRevenue'
import { getCryptoMetricsSelector, queryCryptoMetrics } from '../../services/slices/dashboard/cryptoMetrics'
import { getUserChartSelector, queryUserChart } from '../../services/slices/user/userChart'
import moment from 'moment'
import { cryptoCurrencyOptions, fiatCurrencyOptions, revenueItem } from '../../utils/constants'
...
export default function Dashboard() {
  ...
  const fiatLabels = Object.keys(fiatMetrics?.fiatMetrics || {}).filter(fiatName => fiatName !== 'total').fill('');
  ...
  const cryptoMetricData = Object.keys(cryptoMetrics?.cryptoMetrics || {}).filter(cryptoName => cryptoName !== 'total').map((items) => cryptoMetrics?.cryptoMetrics[items])
  ...
  const cryptoLabels = Object.keys(cryptoMetrics?.cryptoMetrics || {}).filter(cryptoName => cryptoName !== 'total').fill('');
  ...
  const OnApplyFiatFilter = () => {
    ...
    const to = moment(fiatEndDate === null ? new Date() : fiatEndDate).format("YYYY-MM-DD")
    dispatch(queryFiatMetrics({ from, to, currencyShortCode: fiatCurrency })).unwrap()
  }

  const OnApplyCryptoFilter = () => {
    const from = moment(cryptoStartDate).format("YYYY-MM-DD")
    const to = moment(cryptoEndDate === null ? new Date() : cryptoEndDate).format("YYYY-MM-DD")
    dispatch(queryCryptoMetrics({ from, to, currencyShortCode: cryptoCurrency })).unwrap()
  }
  ...
  const FiatCustomInput = forwardRef(({ value, onClick }, ref) => {
    ...
    return (
      <p onClick={onClick} ref={ref} className={' rounded-large border border-dateLine py-2 px-3 cursor-pointer flex items-center gap-2 text-mainColor text-sm font-medium '}>
        {today ? `Today` : value || moment().format('MM/DD/YYYY')} {today ? changeFiatIcon ? <PiCaretUp className='text-mainColor text-xl font-medium' /> : <PiCaretDown className='text-mainColor text-xl font-medium' /> : null}
      </p>
    )
  });
  ...
  const CryptoCustomInput = forwardRef(({ value, onClick }, ref) => {
    ...
    return (
      <p onClick={onClick} ref={ref} className={' rounded-large border border-dateLine py-2 px-3 cursor-pointer flex items-center gap-2 text-mainColor text-sm font-medium '}>
        {today ? `Today` : value || moment().format('MM/DD/YYYY')} {today ? changeCryptoIcon ? <PiCaretUp className='text-mainColor text-xl font-medium' /> : <PiCaretDown className='text-mainColor text-xl font-medium' /> : null}
      </p>
    )
  });
                  ...
                  <div className=' flex items-center gap-5'>
                    <div>
                      <CustomSelect
                        ...
                        dropdownRender={(ReactNodevalue) => {
                          return <div>
                            <ul>
                              {fiatCurrencyOptions.map(items => <li onClick={() => onChangeFiatCurrency(items.value)} className='cursor-pointer'>
                                <div className='flex items-center justify-between px-2'>
                                  <div className='flex items-center gap-2'>
                                    <img src={ngflag} alt='najia flag' className='w-[15px] h-[15px]' />
                                    <p>{items.label}</p>
                                  </div>
                                  <p>{fiatCurrency === items.value ? <GrFormCheckmark color='#E25A5A' /> : null}</p>
                                </div>
                              </li>)}
                    ...
                    <div>
                      <OneDateRange
                        ...
                        endDate={fiatEndDate}
                        maxDate={new Date()}
                        customInput={<FiatCustomInput />}
                        onCalendarOpen={() => setChangeFiatIcon(true)}
                        onCalendarClose={() => setChangeFiatIcon(false)}
                        onApplyFilter={OnApplyFiatFilter}
                        onClearFilter={OnClearFiatFilter}
                  ...
                  <div className=' flex items-center gap-5'>
                    <div>
                      <CustomSelect
                        ...
                        dropdownRender={(ReactNodevalue) => {
                          return <div>
                            <ul>
                              {cryptoCurrencyOptions.map(items => <li onClick={() => onChangeCryptoCurrency(items.value)} className='cursor-pointer'>
                                <div className='flex items-center justify-between px-1'>
                                  <div className='flex items-center gap-1'>
                                    <img src={items.logo} alt={items.label} className='w-[15px] h-[15px]' />
                                    <p>{items.label}</p>
                                  </div>
                                  <p>{cryptoCurrency === items.value ? <GrFormCheckmark color={color.secondaryColor} /> : null}</p>
                                </div>
                              </li>)}
                    ...
                    <div>
                      <OneDateRange
                        ...
                        maxDate={new Date()}
                        customInput={<CryptoCustomInput />}
                        onCalendarOpen={() => setChangeCryptoIcon(true)}
                        onCalendarClose={() => setChangeCryptoIcon(false)}
                        onApplyFilter={OnApplyCryptoFilter}
                        onClearFilter={OnClearCryptoFilter}
                        shouldCloseOnSelect={false}
                        filterStartDate={cryptoStartDate}
